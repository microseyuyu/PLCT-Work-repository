...
| chkconfig network off       | systemctl disable network.service              | 用来检查一个服务在当前环境下被配置为启用还是禁用。 |
...
```
[root@openEuler-riscv64 ~]# systemctl hibernate
Failed to hibernate system via logind: Sleep verb "hibernate" not supported
```

```
[root@openEuler-riscv64 ~]# systemctl hybrid-sleep
